...
import { adaptMiddleware } from "./middlewares/Auth";

const router = Router();

router.get(
  "/software",
  adaptMiddleware(["USER", "ADMIN"]),
  adaptRoute(new SoftwareController(), "findAll")
);
...
router.delete(
  "/requirement/:id",
  adaptRoute(new RequirementController(), "delete")
);
router.patch(
  "/requirement/:id",
  adaptRoute(new RequirementController(), "edit")
);
